...
const writeUp = (
  <section className="prose prose-slate max-w-none">
    ...
    <p>
      Using the IP subnet calculator is straightforward. Enter an IPv4 address in dotted-decimal notation (for example, 192.168.1.100) in the IP address field. Then enter the CIDR prefix length — a number from 0 to 32 — in the prefix field. A /24 means 24 network bits and 8 host bits; a /16 means 16 network bits and 16 host bits. Click Calculate and the IP subnet calculator immediately displays all subnet parameters.
    </p>
    <p>
      You can also enter the subnet mask in dotted-decimal format (255.255.255.0) instead of the CIDR prefix, and the IP subnet calculator converts it automatically. This is useful when you are reading a subnet mask from a network interface configuration and want to understand what CIDR prefix it represents. The tool accepts any valid IPv4 address — public, private, loopback, or link-local — and any valid prefix length.
    </p>
    ...
    <p>
      Subnet masks are used when configuring network interfaces and routing protocols. Wildcard masks are used in Cisco ACLs and OSPF area configurations. In an ACL, a 0-bit in the wildcard means "this bit must match" and a 1-bit means "any value is accepted." So to permit all traffic from the 10.0.0.0/8 network: "permit ip 10.0.0.0 0.255.255.255 any." Our IP subnet calculator shows both masks for every subnet, so you can copy the correct format for your specific use case.
    </p>
    ...
    <p>
      To plan VLSM: list all subnets needed, sorted by host count (largest first). For each subnet, find the smallest CIDR prefix that accommodates the required hosts. Assign subnets from the address space sequentially, making sure ranges do not overlap. Use our IP subnet calculator to verify each subnet's range before assigning it. VLSM is tested heavily on CCNA exams and is the standard practice in enterprise network design.
    </p>

    <h2>IP Subnet Calculator Use for Cloud Architecture (AWS, Azure, GCP)</h2>
    <p>
    ...
    <p>
      Plan your VPC subnets by availability zone and tier. A typical three-tier architecture in two AZs might use: Public-AZ1: 10.0.1.0/24, Public-AZ2: 10.0.2.0/24, App-AZ1: 10.0.11.0/24, App-AZ2: 10.0.12.0/24, DB-AZ1: 10.0.21.0/24, DB-AZ2: 10.0.22.0/24. Leave address space for future subnets by using a /16 VPC — this gives you 256 /24 subnets to allocate. Our IP subnet calculator helps verify that none of these ranges overlap before you commit them in the cloud console.
    </p>

    <h2>IP Subnet Calculator Practice Problems</h2>
    <p>
    ...
    <h2>IPv6 Subnetting and the IP Subnet Calculator: The Future of Network Addressing</h2>
    <p>
    ...
    <p>
      <strong>Using our IP subnet calculator for exam prep</strong>: use our IP subnet calculator to generate practice problems. Enter an IP address and prefix length, note all the outputs, then cover the IP subnet calculator and try to compute each value manually. Check your answers against the IP subnet calculator results. Repeat with varied addresses — especially those near octet boundaries (/8, /16, /24) and in the "interesting" ranges (/17-/23 for the third octet, /25-/30 for the fourth). Practice until you can compute any /24 to /30 subnet in under 30 seconds — the speed needed for exam conditions.
    </p>
