...
const faqs: FaqItem[] = [
  {
    ...
    answer: `A Unicode text converter transforms regular text into stylized alternatives using different Unicode character blocks — all while appearing to be the same "text" and being copyable and pasteable anywhere text is accepted. Unicode contains over 140,000 characters from all the world's writing systems, and among those are multiple different visual representations of the Latin alphabet: mathematical bold letters, italic letters, script (cursive) letters, fraktur (Gothic) letters, double-struck (blackboard bold) letters, circled letters, and more. Our converter takes any text you type and instantly transforms it into these different visual styles: ????????, ????????????, ????????????, ??????????????, ????????????-????????????, and more. Because these are actual Unicode characters (not fonts or images), they paste anywhere that accepts Unicode text.`,
  },
  {
    category: 'Basics',
    question: 'How is Unicode text styling different from regular bold or italic formatting?',
    answer: `Regular bold and italic formatting is applied by software — a word processor, website, or messaging app interprets formatting markup (HTML tags, markdown, rich text) and renders the text visually differently. This formatting is lost when you copy text to platforms that do not support it. Unicode stylized text is different: each stylized character is a distinct Unicode code point that visually resembles a styled version of a Latin letter. The character ?? (U+1D400, MATHEMATICAL BOLD CAPITAL A) looks like a bold A not because bold formatting is applied, but because the character itself is defined as a bold mathematical letter. This means it pastes as "bold" everywhere — Instagram bios, Twitter/X posts, Discord messages, plain text emails, YouTube comments — without any formatting markup. The same text might look like: Twitter ? shows stylized; copy to Notepad ? still stylized; screenshot ? captured as stylized. No platform support is needed because the styling is in the characters themselves.`,
  },
  {
    category: 'Basics',
    question: 'Is Unicode styled text actually text or is it images?',
    answer: `Unicode styled text is genuine text — not images, SVGs, or screenshots. Each stylized character is a real Unicode character that your computer treats as text: it is searchable, selectable, copyable, scalable (it scales with font size without pixelation), and accessible to screen readers (though screen readers may announce the Unicode character name rather than the visual letter). Because it is text, it works in contexts where only plain text is accepted: profile bios on social media that do not allow HTML, plain text email clients, SMS messages, comment fields, and anywhere else text is pasted. This is the key advantage over font-based styling or image-based text generation. When you paste ???????????? text into Twitter, Twitter sees actual Unicode characters, not formatting it needs to render. The platform does not need to "support" the styling — it just needs to support Unicode, which all modern platforms do.`,
  },
  {
    category: 'Styles',
    question: 'What Unicode text styles are available and what do they look like?',
    answer: `Our converter offers 10 distinct Unicode text styles: (1) Bold — thick, heavy weight letters (???????? ????????) using Mathematical Bold Symbols (U+1D400–U+1D433). (2) Italic — slanted letters (???????????? ????????) using Mathematical Italic Symbols (U+1D434–U+1D467). (3) Bold Italic — thick and slanted (???????? ????????????) using Mathematical Bold Italic (U+1D468–U+1D49B). (4) Script — cursive/calligraphic style (????????????) using Mathematical Script Symbols. (5) Fraktur — Gothic/blackletter style (??????????????) using Mathematical Fraktur Symbols. (6) Double-Struck — hollow/outlined letters (????????????-????????????) also called Blackboard Bold. (7) Circled — letters inside circles (???????). (8) Strikethrough — letters with horizontal strike line (S?t?r?i?k?e?t?h?r?o?u?g?h?) using combining diacritics. (9) Underline — letters with underline marks (U_n_d_e_r_l_i_n_e_). (10) Small Caps — smaller uppercase-style letters (S???? C???).`,
  },
  {
    category: 'Usage',
    question: 'How do I use Unicode text on Instagram?',
    answer: `Instagram supports Unicode characters in bios, captions, stories (text input), comments, and display names. To add Unicode styled text to your Instagram bio: type your desired text in our converter, select a style (Bold, Script, etc.), click Copy for the style you want, open Instagram on your phone or browser, go to Edit Profile, tap the Bio field, and paste. The stylized text appears exactly as shown in the converter. Popular Instagram uses: bold headers in bio sections ("?????????? ????"), script-styled quotes, aesthetic formatting with small caps, and mixed styles for visual hierarchy. In captions, Unicode text creates emphasis that stands out without needing to use ALL CAPS. Note that Unicode glyphs render consistently on iOS and Android but may appear slightly differently across devices depending on installed fonts. Test your bio text on both mobile and desktop to check rendering. Special characters like line breaks and formatting may need to be added using the bio editing interface on the app rather than pasted text.`,
  },
  {
    category: 'Usage',
  ...
  },
  {
    ...
    answer: `Aesthetic text (or vaporwave text) typically refers to Unicode styled text (bold, italic, script) or wide-spaced text where regular characters are replaced with full-width Unicode variants (A E S T H E T I C — using the Fullwidth Latin Letters block U+FF01–U+FF60). This creates the characteristic wide-spaced look associated with vaporwave and related aesthetics. Zalgo text is different and more extreme — it uses many stacking combining diacritical marks (marks above and below characters) to create chaotic, glitchy-looking text with characters appearing to overflow their bounding boxes (????__?e????m??o?????n???). Zalgo text is intentionally extreme and difficult to read, used for horror aesthetics, internet humor, and creative shock effects. Our converter focuses on readable aesthetic styles (mathematical alphabet blocks) rather than zalgo effects. Both are Unicode-based, but they use completely different Unicode mechanisms and have very different visual results.`,
  },
  {
    category: 'Technical',
    question: 'What is the Unicode code point for mathematical bold letters?',
    answer: `The Mathematical Alphanumeric Symbols block starts at U+1D400 for MATHEMATICAL BOLD CAPITAL A and proceeds through the alphabet. Capital letters A–Z are U+1D400–U+1D419. Small letters a–z are U+1D41A–U+1D433. Bold italic capital A is U+1D468; bold italic small a is U+1D482. Script capital A is U+1D49C; script small a is U+1D4B6. Fraktur capital A is U+1D504; fraktur small a is U+1D51E. Double-struck capital A is U+1D538; double-struck small a is U+1D552. Sans-serif capital A is U+1D5A0. Monospace capital A is U+1D670. Note that the Unicode standard has some exceptions where specific letters are mapped to characters already existing elsewhere in Unicode — for example, script capital H is U+210B (H), script capital I is U+2110 (I), fraktur capital H is U+210C (H), and fraktur capital I is U+2111 (I). Our converter handles these exceptions automatically.`,
  },
  {
    category: 'Tools',
  ...
  },
  {
    ...
    answer: `The styled text characters used by Unicode text converters come from the Mathematical Alphanumeric Symbols block (U+1D400–U+1D7FF). This block was designed for mathematical notation, not decorative text, but has been repurposed for stylistic effect on social media. Key ranges: Mathematical Bold (U+1D400–U+1D433): ??–??, ??–??, ??–??. Mathematical Italic (U+1D434–U+1D467): ??–??, ??–??. Mathematical Bold Italic (U+1D468–U+1D49B): ??–??, ??–??. Mathematical Script (U+1D49C–U+1D4CF): ??–??, ??–??. Mathematical Bold Script: ??–??, ??–??. Mathematical Fraktur (U+1D504–U+1D537): ??–??. Mathematical Double-Struck (U+1D538–U+1D56B): ??–??, ??–??. Mathematical Sans-Serif (U+1D5A0+): ??–??. Monospace (U+1D670+): ??–??. Note: the Mathematical block has gaps — some code points are allocated to pre-existing characters elsewhere in Unicode (e.g., C for complex numbers at U+2102, not in the Mathematical block). Our converter maps these gaps to the correct pre-existing code points so all 26 letters are always available in each style.`,
  },
  {
    category: 'Technical',
...
const writeUp = (
  <section className="prose prose-slate max-w-none">
    ...
    <p>
      The key insight is that these stylized characters are genuine Unicode text — not images, not fonts, not HTML formatting. When you paste ???????? ???????? into Instagram's bio field, Instagram sees legitimate Unicode characters that happen to look bold. No platform-side formatting support is needed; the styling is inherent to the characters themselves.
    </p>
    ...
    <p>
      Bold text (??????) uses the Mathematical Bold Symbols starting at U+1D400. Italic text (??????) uses Mathematical Italic at U+1D434. Script/cursive (??????) uses Mathematical Script at U+1D49C. Fraktur/Gothic (????C) uses Mathematical Fraktur at U+1D504. Double-Struck (????C) uses Mathematical Double-Struck at U+1D538. Circled letters (???) come from the Enclosed Alphanumerics block at U+24B6. Strikethrough and underline use Unicode combining characters that attach to each letter.
    </p>
    ...
    <p>
      For website content, Unicode styled text should be used with caution from an SEO perspective. Search engines index Unicode characters by their code point values, and they may not equate mathematical bold characters with their standard letter equivalents. A search for the word "bold" on Google will not match ???????? on a webpage using mathematical bold Unicode characters. This means using Unicode styled text for headings, keywords, or important body content could reduce search visibility for those terms.
    </p>
    <p>
    ...
    <p>
      Personal branding applications: influencers and content creators use Unicode bold in their display name across all platforms to create a consistent, visually recognizable presence. The bold name stands out in comment sections, reply threads, and search results compared to standard-weight names. A creator with ???????? ?????????? as their display name catches the eye more readily than Jane Smith in a list of commenters.
    </p>
    <p>
      Business profile optimization: B2B professionals use Unicode styling in LinkedIn headlines and summaries to create emphasis without native formatting support. Consultants often bold their specialty area — ?????????? ???????????????? ?????????????????? — to make their value proposition immediately scannable in search results. LinkedIn's algorithm does not disadvantage Unicode text in profiles, and it renders correctly on both desktop and mobile LinkedIn apps.
    </p>
    <p>
      E-commerce product listings: sellers on platforms like Amazon, Etsy, and eBay use Unicode bold and other styles in product titles and descriptions where the platform does not support HTML formatting. A product listing that opens with ?????????????? ?????????????? in bold Unicode draws attention in search results. However, platform policies vary — some explicitly prohibit Unicode styling in listings, so check terms of service before applying.
    </p>
    <p>
      Event and community promotion: organizers of online events, Discord communities, and Slack workspaces use Unicode styled text in announcements to create visual hierarchy in plain-text environments. An event announcement with the date in ???????? and the venue in ?????????????? is more scannable than the same information in uniform text.
    </p>
    ...
    <p>
      Bold (????????): uses Mathematical Bold characters from the Unicode supplementary plane. The heaviest weight — maximum visual emphasis. Best for: display names, key terms, calls to action, headings in plain-text environments. Platform support: excellent across all modern devices.
    </p>
    <p>
      Italic (????????????): uses Mathematical Italic characters — a slanted, elegant style. Best for: titles of works, foreign phrases, gentle emphasis. Platform support: excellent, though some fonts render these characters with slight style differences.
    </p>
    <p>
      Bold Italic (???????? ????????????): combines weight and slant for the strongest emphasis. Best for: maximum impact headings, important warnings, highlighted key phrases. Use sparingly — heavy visual weight throughout a text reduces impact.
    </p>
    <p>
      Script / Cursive (????????????): uses Mathematical Script characters — an elegant, handwriting-like style. Best for: quotes, bio introductions, aesthetic social media posts, wedding or formal content. Platform support: good on modern devices; may fall back to generic fonts on older Android.
    </p>
    <p>
      Fraktur / Gothic (??????????????): uses Mathematical Fraktur characters — an old-German blackletter style. Best for: fantasy content, historical themes, metal music aesthetics, decorative headers. Platform support: moderate — check appearance on your target audience's devices before using widely.
    </p>
    <p>
      Double-Struck / Blackboard Bold (????????????-????????????): uses Mathematical Double-Struck characters — distinctive outlined letters. Best for: mathematical notation, academic aesthetics, unique usernames, R?????? branding emphasis.
    </p>
    <p>
      Monospace (??????????????????): uses Mathematical Monospace characters — a fixed-width, typewriter-like style. Best for: code references, technical content aesthetics, retro computing themes.
    </p>
    <p>
    ...
    <p>
      NVDA (Windows): when encountering Mathematical Bold character ??, NVDA announces "mathematical bold capital a" — reading out the full Unicode character description rather than just "A." This makes a sentence like "?????????? ??????????" read out as "mathematical bold capital h-e-l-l-o mathematical bold capital w-o-r-l-d" — completely unintelligible as a sentence. Jaws has similar behavior for many Unicode Mathematical characters.
    </p>
    <p>
      VoiceOver (macOS/iOS): VoiceOver on macOS typically reads Mathematical Bold characters as their letter names without the "mathematical bold" qualifier — so ?????????? reads as "Hello." However, this behavior varies by macOS version and may change with future updates. iOS VoiceOver has similar behavior but with some inconsistencies across character ranges.
    </p>
    <p>
...
export default async function UnicodeTextConverterPage() {
  ...
  return (
    <>
      ...
      <ToolPageShell tool={toolData} ui={<UnicodeTextConverterTool />} related={<RelatedTools currentSlug={toolSlug} />}>
        {writeUp}
