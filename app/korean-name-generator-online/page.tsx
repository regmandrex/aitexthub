  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-6 mt-10">
      <div className="prose prose-slate max-w-none">
        ...
        <p><strong>Step 1 â€” Choose gender.</strong> Select male, female, or any gender. The tool has separate name lists for male Korean names and female Korean names so the output matches what you chose.</p>
        <p><strong>Step 2 â€” Choose style.</strong> Select modern, traditional, K-pop/idol, or any style. Modern names reflect names popular in South Korea today. Traditional names include older forms still in use. K-pop/idol names reflect names seen among Korean entertainers and celebrities.</p>
        <p><strong>Step 3 â€” Set the count.</strong> Choose how many names to generate, from 1 to 24 per run.</p>
        <p><strong>Step 4 â€” Enable or disable meanings.</strong> Check the Include meaning box to see the meaning of the given name syllables and the surname. Uncheck it for a clean list of names only.</p>
        <p><strong>Step 5 â€” Click Generate names.</strong> The tool runs in your browser and produces results in under a second.</p>
        <p><strong>Step 6 â€” Copy your results.</strong> Use the Copy button to copy all generated Korean names to your clipboard. Paste into your notes, document, or game.</p>
        ...
        <h2>Korean Name Generator Male â€” Male Korean Names</h2>
        <p>To generate male Korean names, select Male from the gender dropdown. The tool uses a curated list of Korean male given names across modern, traditional, and K-pop styles. Common male Korean given names today include Minjun (ë¯¼ì¤€), Siwoo (ì‹œìš°), Dohyun (ë„í˜„), Seojun (ì„œì¤€), and Junho (ì¤€í˜¸). Traditional male names include Daewon (ëŒ€ì›), Youngho (ì˜í˜¸), and Cheolsu (ì² ìˆ˜). K-pop idol male names seen among popular artists include Taehyung (íƒœí˜•), Jungkook (ì •êµ­), Jimin (ì§€ë¯¼), Baekhyun (ë°±í˜„), and Sehun (ì„¸í›ˆ).</p>
        <p>Male Korean names often use syllables that carry meanings related to talent, brightness, wisdom, or strength. For example, Jun (ì¤€) means talented, Ho (í˜¸) means bright or grand, Min (ë¯¼) means quick or clever, and Hyun (í˜„) means wise. Pairing these syllables with a family name gives a complete Korean male name with meaning.</p>
        ...
        <h2>Korean Name Generator Female â€” Female Korean Names</h2>
        <p>Select Female to generate female Korean names. Female Korean given names in the modern era often use syllables like Yeon (ì—°, lotus), Ji (ì§€, wisdom), Soo (ìˆ˜, long life), Chae (ì±„, colorful), and Eun (ì€, grace). Common modern female names include Jiyeon (ì§€ì—°), Yuna (ìœ ë‚˜), Minji (ë¯¼ì§€), Seoyeon (ì„œì—°), and Jimin (ì§€ë¯¼). Traditional female Korean names include Soonja (ìˆœìž), Younghee (ì˜í¬), and Myungsook (ëª…ìˆ™). K-pop idol female names include Jennie (ì œë‹ˆ), RosÃ© (ë¡œì œ), Irene (ì•„ì´ë¦°), Nayeon (ë‚˜ì—°), and Jihyo (ì§€íš¨).</p>
        <p>Female Korean names tend to use softer or more elegant syllable combinations compared to male names, though this is not a strict rule. The meanings associated with female names often reference nature (lotus, moon, flower), virtues (grace, fidelity, wisdom), or brightness. This Korean name generator female option covers all three style categories â€” modern, traditional, and idol.</p>
        ...
        <p>The meaning display shows two parts: the given name meaning and the surname meaning. For example, Kim Jiyeon (ê¹€ì§€ì—°) would show something like &quot;wisdom and lotus (surname: gold).&quot; This gives you a complete picture of the name. The Korean name generator with meaning option is especially useful for writers who want to match a character's name to their personality, or for anyone curious about Korean naming culture.</p>
        ...
        <p>For example, the surname Kim is ê¹€ in Hangul. The given name Minjun is ë¯¼ì¤€ â€” two syllable blocks, ë¯¼ (min) and ì¤€ (jun). The full name Kim Minjun is written ê¹€ë¯¼ì¤€. This tool outputs both the full romanized name and the full Hangul name for every result.</p>
        ...
        <h2>Korean Idol Name Generator â€” K-pop Style Names</h2>
        ...
        <h2>Korean Name Generator From English â€” How It Works</h2>
        ...
        <p>South Korean naming trends have shifted over generations. Older generations used names like Soonja (ìˆœìž), Kyungsook (ê²½ìˆ™), and Cheolsu (ì² ìˆ˜) that are now considered traditional. Younger generations use names like Minjun (ë¯¼ì¤€), Yuna (ìœ ë‚˜), and Seojun (ì„œì¤€) that rank highly in South Korean baby name statistics. The generator covers both generations, so you can choose the era that fits your creative project.</p>
        ...
        <p>Every name this tool generates is a full Korean name consisting of both the family name (last name) and the given name (first name). In Korean convention, the surname comes first and the given name comes second â€” the opposite of English convention. So Kim Minjun is Mr. Kim, with Minjun being the given name. The tool outputs names in Korean order: surname first.</p>
        <p>The Korean last name generator component draws from the 30 most common Korean surnames, which together cover the majority of the Korean population. Kim, Lee, and Park alone account for roughly 45% of all Koreans. The tool includes less common surnames like Ryu (ë¥˜), Bae (ë°°), and Ha (í•˜) to give variety.</p>
        ...
        <p>The most complete output from this tool shows the romanized name, the Hangul, and the meaning all together. For example: Kim Jiyeon (ê¹€ì§€ì—°) â€” wisdom and lotus (surname: gold). This format is useful for writers who want to include the Korean script in their work, for learners who want to see how the name looks in Hangul, and for anyone who wants to understand what the name means before using it.</p>
        ...
        <p>Tips for using this tool in fiction: run it multiple times and collect 20â€“30 candidates; filter by gender and style appropriate to each character; use the meaning to verify the name fits the character's personality or role; keep Hangul forms for consistency if you include Korean script in your work; avoid accidentally reusing a real Korean celebrity name for a villain (the idol style list uses well-known names as references â€” consider modifying slightly for fictional characters).</p>
        ...
        <p><strong>One syllable surnames:</strong> The vast majority of Korean surnames are one syllable. The most common surnames â€” Kim (ê¹€), Lee (ì´), Park (ë°•), Choi (ìµœ), Jung (ì •) â€” are all monosyllabic. A very small number of Korean surnames are two syllables (e.g., Namgung, ë‚¨ê¶), but these are rare. This tool uses common single-syllable surnames.</p>
        <p><strong>One or two syllable given names:</strong> Korean given names are one or two syllables. Single syllable given names exist (e.g., Joon, ì¤€) but two-syllable given names are more common in modern Korea. This tool generates two-syllable given names for a realistic feel.</p>
...
const pageFaqs: FaqItem[] = [
  ...
  { category: 'General', question: 'Does the Korean name generator show Hangul?', answer: 'Yes. Every name generated by this tool includes the Hangul (í•œê¸€) script alongside the romanized spelling. For example, Kim Minjun is shown as ê¹€ë¯¼ì¤€. This makes the tool useful for writers who need the Korean script, language learners, and anyone who wants to see how the name looks in actual Korean.' },
  ...
  { category: 'General', question: 'What Korean surnames does the generator use?', answer: 'The tool uses the 30 most common Korean surnames including Kim (ê¹€), Lee (ì´), Park (ë°•), Choi (ìµœ), Jung (ì •), Kang (ê°•), Cho (ì¡°), Yoon (ìœ¤), Jang (ìž¥), Lim (ìž„), and more. These surnames cover the majority of the Korean population, so every generated name has a realistic family name.' },
  { category: 'General', question: 'How are Korean names structured?', answer: 'Korean names consist of a one-syllable surname (family name) followed by a one or two syllable given name. The surname always comes first in Korean convention. For example, Kim Minjun â€” Kim is the surname, Minjun is the given name. In Hangul: ê¹€ë¯¼ì¤€. Total length is typically 2â€“3 syllables.' },
  ...
  { category: 'Limits', question: 'How many Korean names can I generate?', answer: 'You can generate 1â€“24 names per run. There is no daily or total limit. Run the generator as many times as you like. Each run produces a new random set. Paste multiple runs into one document if you need a large pool of names.' },
  ...
  { category: 'General', question: 'What is the difference between modern and traditional Korean names?', answer: 'Modern Korean names are those popular in South Korea today â€” names like Minjun, Yuna, Seojun, and Jiyeon that rank highly in current baby name statistics. Traditional names include older forms like Soonja, Cheolsu, and Youngho that were common in previous generations and are still used. The K-pop/idol style reflects names associated with Korean entertainers.' },
