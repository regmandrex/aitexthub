  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-6 mt-10">
      <div className="prose prose-slate max-w-none">
        ...
        <p>Korean names follow a specific structure. The family name (성, seong) comes first. Common Korean surnames include Kim (김), Lee (이), Park (박), Choi (최), and Jung (정). The given name (이름, ireum) follows, usually consisting of one or two Hangul syllable blocks. Each syllable block is made of consonants and vowels. The tool generates both the romanized spelling and the Hangul so you see exactly how the name looks and sounds.</p>
        ...
        <p>To generate male Korean names, select Male from the gender dropdown. The tool uses a curated list of Korean male given names across modern, traditional, and K-pop styles. Common male Korean given names today include Minjun (민준), Siwoo (시우), Dohyun (도현), Seojun (서준), and Junho (준호). Traditional male names include Daewon (대원), Youngho (영호), and Cheolsu (철수). K-pop idol male names seen among popular artists include Taehyung (태형), Jungkook (정국), Jimin (지민), Baekhyun (백현), and Sehun (세훈).</p>
        ...
        <p>Select Female to generate female Korean names. Female Korean given names in the modern era often use syllables like Yeon (연, lotus), Ji (지, wisdom), Soo (수, long life), Chae (채, colorful), and Eun (은, grace). Common modern female names include Jiyeon (지연), Yuna (유나), Minji (민지), Seoyeon (서연), and Jimin (지민). Traditional female Korean names include Soonja (순자), Younghee (영희), and Myungsook (명숙). K-pop idol female names include Jennie (제니), Rosé (로제), Irene (아이린), Nayeon (나연), and Jihyo (지효).</p>
        ...
        <p>South Korean naming trends have shifted over generations. Older generations used names like Soonja (순자), Kyungsook (경숙), and Cheolsu (철수) that are now considered traditional. Younger generations use names like Minjun (민준), Yuna (유나), and Seojun (서준) that rank highly in South Korean baby name statistics. The generator covers both generations, so you can choose the era that fits your creative project.</p>
...
const pageFaqs: FaqItem[] = [
  ...
  { category: 'General', question: 'What Korean surnames does the generator use?', answer: 'The tool uses the 30 most common Korean surnames including Kim (김), Lee (이), Park (박), Choi (최), Jung (정), Kang (강), Cho (조), Yoon (윤), Jang (장), Lim (임), and more. These surnames cover the majority of the Korean population, so every generated name has a realistic family name.' },
