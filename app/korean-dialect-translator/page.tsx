      ...
      <div className="mx-auto w-full max-w-3xl px-4 py-5 min-h-screen sm:py-8 md:py-10">
        ...
        <section className="mt-10 space-y-10 text-slate-700 leading-relaxed">
          ...
          <div className="space-y-4">
            ...
            <p>경상도 사투리는 부산, 대구, 울산, 경주, 포항 등에서 사용되며 강하고 직설적인 어조가 특징입니다. 사투리 번역기에서 경상도를 선택하면 다음과 같은 어미와 단어가 자동으로 변환됩니다. "~하세요" → "~하이소", "~했어요" → "~했데이", "~입니다" → "~입니더", "~습니다" → "~심더", "왜" → "와", "너무" → "억수로", "지금" → "시방", "맞아" → "맞데이"입니다. 이 규칙들은 경상도 드라마, 영화, 예능에서 가장 자주 등장하는 표현을 기준으로 정리되었습니다.</p>
          ...
          <div className="space-y-4">
            ...
            <p>전라도 사투리는 광주, 전주, 목포, 순천 등에서 사용되며 부드럽고 정감 있는 어조가 특징입니다. 사투리 번역기에서 전라도를 선택하면 "~하세요" → "~허씨요", "~했어요" → "~혔어라", "~입니다" → "~이당께", "~습니다" → "~당께", "그러니까" → "긍께", "너무" → "겁나", "정말" → "징하게", "빨리" → "싸게" 같은 변환이 적용됩니다.</p>
          ...
          <div className="space-y-4">
            ...
            <p>충청도 사투리는 대전, 청주, 천안 등에서 사용되며 천천히 말하는 느린 어조와 어미를 길게 끄는 발음이 특징입니다. 사투리 번역기에서 충청도를 선택하면 "~하세요" → "~하셔유", "~했어요" → "~했슈", "~이에요" → "~이유", "~예요" → "~유", "~해요" → "~햐", "정말" → "참말로", "빨리" → "얼른", "맞아" → "맞아유" 같은 변환이 적용됩니다.</p>
          ...
          <div className="space-y-4">
            ...
            <p>제주도 사투리는 가장 독특합니다. 사실 학계에서는 제주도 사투리를 "제주어"로 분류해 별도의 언어로 취급하기도 하며, 유네스코는 제주어를 "소멸 위기 언어"로 등록했습니다. 사투리 번역기에서 제주도를 선택하면 "~하세요" → "~허우꽈", "~했어요" → "~핸수다", "~이에요" → "~우다", "~습니다" → "~수다", "왜" → "무사", "너무" → "하영", "안녕" → "혼저옵서", "맞아" → "맞수다" 같은 변환이 적용됩니다.</p>
