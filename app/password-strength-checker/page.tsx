...
const faqs: FaqItem[] = [
  ...
  },
  {
    ...
    answer: `No. Our password strength checker runs entirely in your browser using client-side JavaScript. Your password is analyzed locally on your device and is never transmitted to our servers, never stored in any database, never logged in analytics, and never shared with any third party. The analysis code processes the password in your browser's JavaScript engine, and once you close or reload the tab, the password disappears from memory. This is a fundamental design principle: a password strength checker that sends passwords to a server — even over HTTPS — would be a security risk, because it creates a record of passwords that could be accessed or leaked. We intentionally use a browser-only architecture for this password strength checker precisely to provide ironclad privacy guarantees. You can verify this claim: open your browser's developer tools (F12), go to the Network tab, and watch — no network requests are made when you type your password into our password strength checker.`,
  },
  {
    category: 'Tools',
...
const writeUp = (
  <section className="prose prose-slate max-w-none">
    ...
    <h2>How Our Password Strength Checker Evaluates Passwords</h2>
    <p>
    ...
    <h2>Password Strength Checker Science: Length vs. Complexity</h2>
    <p>
    ...
    <h2>Password Attacks the Password Strength Checker Defends Against: Brute Force, Dictionary, and Credential Stuffing</h2>
    <p>
    ...
    <p>
      Creating a strong password is necessary but not sufficient — you also need to ensure your password has not appeared in known data breaches. The Have I Been Pwned (hibp) service maintains a database of over 12 billion compromised passwords from real breaches. Their password checking API uses k-anonymity to let you check if a specific password appears in breach data without sending the actual password to the server. Major password managers integrate this check automatically. We recommend checking any new password you create against this database before using it — our password strength checker focuses on mathematical strength; breach database checking covers the orthogonal risk of reusing compromised passwords even when they appear to be original.
    </p>
    ...
    <h2>Common Password Vulnerabilities a Password Strength Checker Flags</h2>
    <p>
    ...
    <p>
      Entering your actual password into any online tool requires trust that the password strength checker does not capture and transmit it. Our password strength checker is built from the ground up for privacy — all analysis happens in your browser using JavaScript. No passwords are sent to our servers; no network requests are made when you type in the password field. You can verify this yourself using your browser's network monitoring tab (F12 ? Network) — typing a password in our password strength checker generates zero network requests.
    </p>
    <p>
