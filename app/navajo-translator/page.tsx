  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-6 mt-10">
      <div className="prose prose-slate max-w-none">
        ...
        <p>If you are preparing text for a document or project, you may use several tools in sequence. Paste content from a webpage into a strip-HTML tool first, then use a space remover to clean extra spaces. Once your text is clean, you can run it through the Navajo translator.</p>

        <h2>Similar Period and Language Translators</h2>
        <p>If you need period or stylistic English instead of Navajo, we have other translator pages. The <Link href="/medieval-translator">medieval translator</Link> and <Link href="/middle-english-translator">Middle English translator</Link> target Chaucer-era language; the <Link href="/old-english-translator">Old English translator</Link> handles Anglo-Saxon (translate to Anglo Saxon); the <Link href="/shakespearean-translator">Shakespearean translator</Link> produces Early Modern “thee and thou” style. We also offer a <Link href="/fancy-english-translator">fancy English translator</Link> for decorative wording. For the full tool list, see the <Link href="/">homepage</Link>. Each tool fits a different era or use case—pick the one that matches your project.</p>
        ...
        <p>We also offer period and stylistic English translators: medieval translator, Middle English translator, Old English translator (translate to Anglo Saxon), Shakespearean translator, and fancy English translator. Each fits a different use case. The homepage lists all tools and text utilities including strip HTML and space remover for cleaning pasted content.</p>
        ...
        <p>Use a Navajo translator when you need English to Navajo support for learning, education, or respectful exploration. The tool is free, runs in the browser, and does not require sign-up. Treat the output as a learning aid; for formal or official use, use certified translators and resources from the Navajo Nation when needed. The homepage lists all tools.</p>

        <h2>Quick Reference: Navajo Translator and Related Tools</h2>
        <p>Use the Navajo translator when you need English to Navajo support. For period or stylistic English, try the medieval translator, Middle English translator, Old English translator (translate to Anglo Saxon), Shakespearean translator, or fancy English translator. The homepage has the full list of tools.</p>
        ...
        <p>Before you use or share output from the Navajo translator: Is the use appropriate (learning and support only)? Did you clean pasted input with a strip-HTML and space-remover tool if it came from the web? For official or certified Navajo, have you directed users to professional resources? Use the Navajo translator for learning and respectful use only.</p>
        ...
        <p>If you are preparing materials that include both English and Navajo, you may use several tools. Paste content from a webpage into a strip-HTML tool first, then use a space remover to clean extra spaces. Use the Navajo translator for the phrases you want to convert to Navajo. For period or stylistic English (e.g. translate into Shakespearean, translate to Anglo Saxon), we have dedicated translator pages on our homepage.</p>

        <h2>English to Navajo: Copy-Paste Workflow</h2>
        <p>When you copy text from a webpage or document into the Navajo translator, clean it first. Paste into a strip-HTML tool to remove markup, then use a space remover to normalize spaces and line breaks. Once the text is plain and tidy, paste it into the English to Navajo translator. That workflow keeps the input clean and the output easier to use. After you run text through the Navajo translator, you may use the result for learning or support. If you combine it with other content from the web, run that content through strip HTML and space remover too so the final document is consistent.</p>
        ...
        <p>For best results with the Navajo translator: start with clean, plain text. If you pasted from the web, use a strip-HTML and space-remover tool first. Paste into the English to Navajo translator and click translate. Copy the output and use it for learning or support; for official or certified use consult professional translators. For period or stylistic English—medieval, Middle English, Old English, Shakespearean, fancy English—see our other translator pages on the homepage. That keeps the input tidy and the output easier to use for learning and practice.</p>
...
export default async function NavajoTranslatorPage() {
  ...
  const pageFaqs: FaqItem[] = [
    ...
    { category: 'Use cases', question: 'Can I translate from Navajo to English?', answer: 'This tool focuses on converting English to Navajo. Some other tools offer reverse conversion from Navajo to English. For cleaning and formatting text before or after you translate, use a space remover or strip-HTML utility.' },
    { category: 'General', question: 'Navajo translator vs other language tools?', answer: 'We also have period and stylistic English translators: medieval translator, Middle English translator, Old English translator (translate to Anglo Saxon), Shakespearean translator, and fancy English translator. Each targets a different style or era; the Navajo translator is for English to Navajo (Diné bizaad) support. The homepage lists all tools.' },
    ...
    { category: 'General', question: 'Where can I find an English to Navajo tool?', answer: 'This page is our English to Navajo translator. For period English style—Middle English, Old English (translate to Anglo Saxon), Shakespearean, or medieval—see our other translator pages. The homepage lists all period and language tools.' },
    ...
    { category: 'General', question: 'Where can I find more translator tools?', answer: 'Our homepage lists similar period and language tools: medieval translator, Middle English translator, Old English translator (translate to Anglo Saxon), Shakespearean translator, and fancy English translator. Each targets a different era or style—pick the one that matches your project.' },
  ];
