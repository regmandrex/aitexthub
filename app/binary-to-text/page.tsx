...
const writeUp = (
  <section className="prose prose-slate max-w-none">
    ...
    <p>
      Understanding binary encoding is foundational in computer science — it reveals how computers actually store and process text at the hardware level. It is also a frequently encountered encoding in CTF challenges, programming exercises, escape rooms, and educational puzzles. Our binary to text converter makes the conversion instant while the article below explains the underlying concepts thoroughly.
    </p>
    ...
    <p>
      CTF binary challenges: Capture the Flag competitions frequently include binary encoding challenges as a baseline cryptography or encoding puzzle. Recognizing binary encoding patterns (groups of 8 bits with values between 00000000 and 01111111 for ASCII characters) is a fundamental CTF skill. Our binary to text converter is among the most-used tools in CTF competitions for exactly this purpose.
    </p>
    ...
    <p>
      <strong>Whitespace and invisible characters</strong> in binary strings can cause parsing failures. Copy-pasting binary from some sources introduces non-breaking spaces (U+00A0), zero-width spaces (U+200B), or other Unicode whitespace characters that look like regular spaces but aren't. These cause the split operation to fail silently. Our binary to text converter normalizes whitespace before parsing, but if implementing manually you should trim and normalize input first.
    </p>
    <p>
