  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-6 mt-10">
      <div className="prose prose-slate max-w-none">
        ...
        <h2>Conclusion</h2>
        <p>Whether you need to merge lines into one paragraph, remove newlines from PDF or spreadsheet text, or prepare clean single-line text for forms and SEO, a line break remover saves time and keeps formatting consistent. Use this free remove line breaks online tool to strip or replace line breaks in seconds, with options to preserve paragraph structure when needed. For more text cleanup—extra spaces, HTML tags, or invisible characters—combine it with our other utilities such as the space remover or strip HTML tool for a full workflow.</p>
      </div>
    </section>
  );
}
...
export default async function RemoveLineBreaksPage() {
  ...
  const pageFaqs: FaqItem[] = [
    ...
    { category: 'Compatibility', question: 'Does the tool work on mobile?', answer: 'Yes. The remove line breaks tool runs in your browser, so it works on phones and tablets. Paste text (e.g., from notes or email), run the tool, and copy the result. No app install is required.' },
