...
const faqs: FaqItem[] = [
  ...
  },
  {
    ...
    answer: 'False positives on the C2PA Content Credentials layer are essentially impossible because the manifest is cryptographically signed by Adobe; a positive detection is definitive. False negatives are common when Content Credentials have been stripped by social media uploads or non-Adobe video editors. A "no watermark" result means Content Credentials were absent or removed before you received the file, not that the video definitely isn&#39;t from Firefly. Pixel-level signal detection on sampled frames is heuristic and reports confidence levels for ambiguous cases.',
  },
  {
    category: 'Reporting',
    question: 'What does the Firefly video detector report show?',
    answer: 'The detector reports: (1) C2PA Content Credentials manifest presence with Adobe as the signer, including video-specific assertions (frame count, duration, generation parameters, edit history); (2) XMP metadata in Adobe namespaces; (3) container-level metadata (mp4 atoms, mov boxes); (4) optional pixel-level signal analysis on sampled frames. The report distinguishes "C2PA Detected — Verified" (manifest valid and unmodified) from "C2PA Detected — Modified" (manifest valid but content was edited after generation).',
  },
  {
    category: 'Workflow',
    question: 'How do I integrate Firefly video detection into a content workflow?',
    answer: 'For per-video checks, the browser tool is suitable. For automated workflows, use the c2patool CLI and the c2pa-rs / c2pa-python libraries for programmatic Content Credentials reading; ffprobe extracts container-level metadata. A typical pipeline runs C2PA verification first (definitive when present), then ExifTool inspection, then optionally a pixel-level frame analysis as a fallback for files with stripped Content Credentials.',
  },
];
