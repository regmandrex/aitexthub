    ...
    <div className="space-y-4">
      <div className="flex gap-2 items-start">
        <div className="flex-1">
          ...
          <div className="flex items-center rounded-lg border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-blue-500">
        ...
        <div className="pt-5">
          <div className="flex gap-1">
            {allFlags.map(f => (
              <button key={f} onClick={() => setFlags(prev => prev.includes(f) ? prev.replace(f, '') : prev + f)} className={`w-7 h-7 rounded text-xs font-mono font-medium transition-colors ${flags.includes(f) ? 'bg-blue-600 text-white' : 'border border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{f}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea value={text} onChange={e => setText(e.target.value)} rows={5} placeholder="Enter text to test against..." className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-mono resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" />
      </div>
      ...
      {result && !result.error && text && (
        <div className="space-y-3">
          ...
          {highlighted && (
            <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-mono whitespace-pre-wrap break-words">
              {highlighted.map((p, i) => (
