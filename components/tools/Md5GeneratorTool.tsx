...
function md5(input: string): string {
  ...
  const ustr = unescape(encodeURIComponent(input));
  return binl2hex(binlMD5(str2binl(ustr), ustr.length * 8));
}
    ...
    <div className="space-y-4">
      <div>
        ...
        <textarea
          ...
          className="w-full rounded-lg border-3 border-black px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
      <button
        onClick={generate}
      ...
      {hash && (
        <div className="space-y-2">
          <label className="block text-sm font-medium text-slate-700">MD5 Hash</label>
          <div className="flex items-center gap-2">
            <code className="flex-1 rounded-lg border-3 border-black bg-slate-50 px-3 py-2 text-sm font-mono break-all">{hash}</code>
            <button
              onClick={copy}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${copied ? 'bg-green-500 text-white' : 'bg-blue-600 hover:bg-blue-700 text-white'}`}
            >
              {copied ? '✓ Copied!' : 'Copy'}
            </button>
          </div>
