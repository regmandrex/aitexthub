    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 text-sm">
        ...
        <input type="number" value={base} onChange={e => setBase(Number(e.target.value) || 16)} min={1} max={32}
          className="w-20 rounded-lg border-3 border-black px-2 py-1.5 text-center text-sm focus:outline-none focus:ring-2 focus:ring-blue-100" />
      ...
      {tab === 'pxToRem' ? (
        <div className="flex items-center gap-3">
          <input type="number" value={px} onChange={e => setPx(e.target.value)} placeholder="px value"
            className="w-32 rounded-xl border-3 border-black px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-100" />
          ...
          {pxResult && (
            <button onClick={() => { navigator.clipboard.writeText(pxResult); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
              className="text-xs px-2 py-1 rounded-lg border-3 border-black bg-white hover:bg-slate-50">{copied ? 'Copied!' : 'Copy'}</button>
          )}
        </div>
      ) : (
        <div className="flex items-center gap-3">
      ...
      <div className="border-t-2 border-black pt-4 space-y-3">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <textarea value={bulk} onChange={e => setBulk(e.target.value)} placeholder={"8\n12\n16\n24\n32"} rows={6}
            className="rounded-xl border-3 border-black px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-100" />
          <textarea value={bulkOut} readOnly rows={6}
            className="rounded-xl border-3 border-black px-3 py-2 text-sm font-mono bg-slate-50" />
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-slate-50">
                <th className="text-left px-3 py-2 text-xs font-semibold text-slate-600 border-3 border-black">PX</th>
                <th className="text-left px-3 py-2 text-xs font-semibold text-slate-600 border-3 border-black">REM</th>
                <th className="text-left px-3 py-2 text-xs font-semibold text-slate-600 border-3 border-black">PX (base 14)</th>
                <th className="text-left px-3 py-2 text-xs font-semibold text-slate-600 border-3 border-black">PX (base 18)</th>
              </tr>
            </thead>
            <tbody>
              {COMMON_PX.map(p => (
                <tr key={p} className="hover:bg-slate-50">
                  <td className="px-3 py-1.5 border-2 border-black font-mono text-slate-800">{p}px</td>
                  <td className="px-3 py-1.5 border-2 border-black font-mono text-blue-700">{(p/base).toFixed(4).replace(/\.?0+$/,'')}rem</td>
                  <td className="px-3 py-1.5 border-2 border-black font-mono text-slate-500">{(p/14).toFixed(4).replace(/\.?0+$/,'')}rem</td>
                  <td className="px-3 py-1.5 border-2 border-black font-mono text-slate-500">{(p/18).toFixed(4).replace(/\.?0+$/,'')}rem</td>
                </tr>
              ))}
            </tbody>
          </table>
