...
const THEME_PROMPTS: Record<string, string> = {
  ...
  'funny': 'Generate {count} unique funny names. Use a mix of comedy mechanics: puns on real names, mismatched word pairings, alliteration, and absurdly grand titles on trivial things. Each name should land a distinct joke rather than just sounding random. Return only the names, one per line.',
  'punny': 'Generate {count} unique punny names. Every single one must be a genuine pun on a real word, phrase, food, or common expression, built to read as a plausible person name. Vary the pun type across homophones, split-word puns, and near-miss respellings. Return only the names, one per line.',
  'stupid': 'Generate {count} unique stupid-sounding names. Blunt, dumb, ridiculous, and a bit crude in energy but never slurs or explicit content. Lean on dopey syllables, hard consonants, and names that sound like an idiot character. Return only the names, one per line.',
