...
export function MhaOcMakerTool() {
  ...
  const generate = async () => {
    ...
    const nextSheet: MhaSheet = { name: concept.trim() || 'Original Hero OC', quirk: `${quirkType} Quirk with a ${weakness.toLowerCase()}.`, role, costume: `${costume}, ${color}, ${supportItem}.`, personality };
    if (!isPro) {
  ...
  return <>
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3"><button type="button" onClick={generate} disabled={isGenerating} className="rounded-lg bg-brand-700 px-5 py-2.5 text-sm font-semibold text-white shadow hover:bg-brand-800 disabled:cursor-not-allowed disabled:opacity-50">{isGenerating ? 'Creating your hero...' : freeConceptUsed && !isPro ? 'Unlock image generation' : 'Make my MHA OC'}</button><button type="button" onClick={copySheet} disabled={!sheet} className="rounded-lg border-3 border-black bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">Copy character sheet</button><button type="button" onClick={() => setSheet(null)} disabled={!sheet} className="rounded-lg border-3 border-black bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">Clear</button></div>
      {error && <div className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}
      {sheet && <article className="max-w-2xl rounded-xl border-3 border-black bg-slate-50 p-4"><div className="mb-3 flex items-start justify-between gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-100 text-sm font-bold text-brand-800">1</span><span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Your MHA OC</span></div>{sheet.image ? <img src={sheet.image} alt={`${sheet.name} My Hero Academia OC concept`} className="mb-4 aspect-square w-full rounded-lg border-3 border-black object-cover" /> : <button type="button" onClick={() => { if (session?.user) setPricingOpen(true); else setAuthOpen(true); }} className="mb-4 w-full rounded-lg border border-brand-200 bg-white px-3 py-3 text-sm font-semibold text-brand-800 hover:bg-brand-50">Unlock image generation</button>}<h3 className="text-lg font-semibold text-slate-900">{sheet.name}</h3><dl className="mt-3 space-y-2 text-sm text-slate-700"><div><dt className="font-semibold text-slate-900">Quirk</dt><dd>{sheet.quirk}</dd></div><div><dt className="font-semibold text-slate-900">Role</dt><dd>{sheet.role}</dd></div><div><dt className="font-semibold text-slate-900">Costume</dt><dd>{sheet.costume}</dd></div><div><dt className="font-semibold text-slate-900">Personality</dt><dd>{sheet.personality}</dd></div></dl></article>}
    </div>
