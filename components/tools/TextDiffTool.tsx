    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          ...
          <textarea value={left} onChange={e => setLeft(e.target.value)} rows={8} placeholder="Paste original text..." className="w-full rounded-lg border-3 border-black px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Modified text</label>
          <textarea value={right} onChange={e => setRight(e.target.value)} rows={8} placeholder="Paste modified text..." className="w-full rounded-lg border-3 border-black px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
      </div>
      ...
      {diff && (
        <div className="space-y-2">
          ...
          <div className="rounded-lg border-3 border-black bg-white px-4 py-3 text-sm leading-relaxed font-mono whitespace-pre-wrap break-words">
            {diff.map((part, i) => (
