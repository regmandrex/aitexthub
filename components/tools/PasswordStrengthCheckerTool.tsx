    ...
    <div className="space-y-5">
      <div className="relative">
        <input
          ...
          className="w-full rounded-xl border border-slate-200 px-4 py-3 pr-20 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-100 font-mono"
        />
        <button onClick={() => setShow(!show)}
          className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 hover:text-slate-700 px-2 py-1 border border-slate-200 rounded-lg bg-white">
          {show ? 'Hide' : 'Show'}
        </button>
      </div>
          ...
          {generated && (
            <>
              ...
              <button onClick={() => { navigator.clipboard.writeText(generated); setCopiedGen(true); setTimeout(() => setCopiedGen(false), 1500); }}
                className="text-xs px-2 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-slate-50">
